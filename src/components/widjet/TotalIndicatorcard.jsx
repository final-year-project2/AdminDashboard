...
function TotalIndicatorcard() {
        const { data:TotalTransactionData,isPending,error } = useGetData('http://127.0.0.1:8000/adminOperation/Total_trunsaction/')
        ...
        if(TotalTransactionData && TotalTransactionData.length !=0){
            data = [
                {
                name: 'day 1 to 7',
                uv: TotalTransactionData.start_of_1_7,
                pv: 2400,
                amt: 160,
                },
                {
...
return (
    <div className='m-10'>
        {
            TotalTransactionData && <div className='flex    text-gray-500'>
            <div className='flex-none w-1/3  mr-5 p-3 shadow-md rounded-sm '>
                <div className='flex justify-between font-semibold'>
                    <div>Total Revenue</div>
                    <div><KeyboardDoubleArrowDownIcon fontSize='25' className='text-green-400 '/></div>
                </div>
        
                <div className='flex justify-center'>
                    <div className='w-32 pt-5'>
                    <CircularProgressbar text={`${(TotalTransactionData.total_Transaction/100)*100}%`} strokeWidth={5} value={70} />
                    </div>
                </div>
                ...
                <div className=' text-center pb-5 px-3 '>
                    ...
                    <p >Previes transactions processing last payment may not be include</p>
                </div>
                ...
                <div className='flex justify-between text-center px-3'>
                    ...
                    <div>
                        ...
                        <div className='text-xl font-bold'>
                            <ArrowRightAltIcon fontSize=''/>
                            {target-TotalTransactionData.total_Transaction} <span className='font-thin text-base'>Birr</span>
                        </div>
                    </div>
                </div>
                </div>
